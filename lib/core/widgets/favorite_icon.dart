...
class FavoriteIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MultiBlocListener(
      listeners: [
        BlocListener<AddEventToFavoritesCubit, AddEventToFavoritesState>(
        ...
        BlocListener<
          RemoveEventFromFavouritesCubit,
          RemoveEventFromFavouritesState
        >(
          listener: (context, state) {
            if (state is RemoveEventFromFavouritesFailure) {
              CustomSnackBar(
                type: SnackBarType.failure,
                title: S.of(context).oops,
                subTitle: state.errMessage,
            ...
            } else if (state is RemoveEventFromFavouritesSuccess) {
              CustomSnackBar(
                type: SnackBarType.success,
                title: S.of(context).success,
                subTitle: S.of(context).event_removed_from_Favorite,
                context: context,
              );
            }
          },
